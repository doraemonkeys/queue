...
func TestPushHeapTopK4(t *testing.T) {
	...
	t.Run("Full heap, add larger", func(t *testing.T) {
		...
		if len(h) != 3 || h[0] != 5 || !IsHeap(h, less) {
	...
	t.Run("Full heap, add smaller", func(t *testing.T) {
		h := []int{3, 7, 5}
		less := func(a, b int) bool { return a < b }
		PushHeapTopK(&h, 2, less, 3)
		if len(h) != 3 || h[0] != 3 || !IsHeap(h, less) {
	...
	t.Run("k = 1", func(t *testing.T) {
		h := []int{5}
		less := func(a, b int) bool { return a < b }
		PushHeapTopK(&h, 3, less, 1)
		if len(h) != 1 || h[0] != 5 {
			t.Errorf("Expected [5], got %v", h)
		}
		PushHeapTopK(&h, 7, less, 1)
		if len(h) != 1 || h[0] != 7 {
